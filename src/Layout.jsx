...
const Icon = {
  ...
  X: (p) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" {...p}><path d="M5 5l14 14M19 5L5 19"/></svg>,
    ...
    <nav className={`nav ${scrolled ? "scrolled" : ""}`}>
      <div className="container nav-row">
        ...
        <div className="nav-links">
          {links.map((l) => (
            <span
              ...
              onClick={() => { navigate(l.route); if (l.hash) setTimeout(() => document.querySelector(l.hash)?.scrollIntoView({ behavior: "smooth", block: "start" }), 50); }}
            >{l.label}</span>
          ))}
        </div>
        <div className="nav-tools">
          ...
          <button className="icon-btn" aria-label="Account" style={{ display: window.innerWidth > 720 ? "grid" : "none" }}><Icon.User /></button>
          <button className="icon-btn" aria-label="Cart" onClick={() => navigate("cart")}>
            <Icon.Bag />
            {cartCount > 0 && <span className="cart-count">{cartCount}</span>}
          </button>
          <button className="btn btn-primary" onClick={() => navigate("catalog")} style={{ padding: "10px 18px", fontSize: 13 }}>
